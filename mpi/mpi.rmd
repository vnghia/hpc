...
---
```{r, chunk-mpi, include=F, cache=F}
knitr::read_chunk(here::here("mpi", "mpi.r"))
```

```{r, init-mpi, include=F}
```
...
```{r, B-dense, echo=F}
```
```{r, B-dense-plot, echo=F, fig.cap=""}
b_plot
```
Compared to the dense version, the $B^{\intercal}$-optimized version takes less time to compute in general, which means our algorithm is correct.
