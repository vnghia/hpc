...
class BinaryOpenMP:
    ...
    def __has_result(self: BinaryOpenMP):
        return self.time is not None and self.norm != 0 and self.gflops is not None
    ...
    def __compile(
        ...
        if self.commit != "HEAD":
            self.hash = None
            subprocess.run(["git", "checkout", self.commit, self.source_path])
